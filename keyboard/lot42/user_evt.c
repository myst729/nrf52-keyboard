...
// #define COLOR_SCROLL_LOCK
#define COLOR_CAPS_LOCK 0xFF0000 // 红色: 大写锁定
// #define COLOR_NUM_LOCK
...
/**
 * @brief 按锁定状态更改 LED 样式
 * 
 */
static void led_key_lock_change(uint8_t led)
{
    if (led & 0x04) {
#ifdef COLOR_SCROLL_LOCK
        keyboard_led_rgb_set(COLOR_SCROLL_LOCK);
#endif
    }
    if (led & 0x02) {
#ifdef COLOR_CAPS_LOCK
        keyboard_led_rgb_set(COLOR_CAPS_LOCK);
#endif
    }
    if (led & 0x01) {
#ifdef COLOR_NUM_LOCK
        keyboard_led_rgb_set(COLOR_NUM_LOCK);
#endif
    }
}
